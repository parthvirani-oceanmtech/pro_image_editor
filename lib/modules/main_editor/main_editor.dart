...
class ProImageEditorState extends State<ProImageEditor>
  ...
  void _decodeImage() async {
    ...
    var decodedImage = await decodeImageFromList(await _stateManager.image.safeByteArray);
  ...
  void openCropEditor() async {
    ...
    if (activeLayers.isNotEmpty || _stateManager.filters.isNotEmpty || _stateManager.blurStateHistory.blur != 0) {
      _activeCrop = true;
    ...
    _openPage<CropRotateEditorRes?>(
      ...
      if (response != null) {
        ...
        if (res.bytes != null) {
          ...
          bool oldFitWidth = _screenSize.imageWidth >= _screenSize.screen.width - 0.1 &&
              _screenSize.imageWidth <= _screenSize.screen.width + 0.1;
          bool newFitWidth = newImgW >= _screenSize.screen.width - 0.1 && newImgW <= _screenSize.screen.width + 0.1;
